    ...
    return (
        <header className='bg-tuition-care-base fixed w-full'>
            <div className='w-4/5 mx-auto text-white py-4 flex justify-between items-center'>
                ...
                <nav className={`absolute md:static w-full md:w-fit p-5 md:p-0 bg-tuition-care-base left-1/2 -translate-x-1/2 md:translate-x-0 duration-500 ${menuClick ? 'top-10' : 'top-[-250px]'}  flex flex-col md:flex-row justify-center md:items-center -z-50`}>
                    <Link className='hover:opacity-40 duration-300 mt-5 md:mt-0' to='/'>Home</Link>
                    <Link className='hover:opacity-40 duration-300 mt-2 md:mt-0 md:ml-5' to='/services'>Services</Link>
...

                    {
                        user ?
                            <>
                                <p className='font-bold mt-6 md:mt-10 ml-0 md:ml-10'>{user.displayName}</p>
                                <button
                                    onClick={handleSignOut}
                                    className='mt-5 md:ml-5 hover:opacity-40 duration-300 bg-tuition-care-base-light px-5 py-2 rounded-lg'>
                                    Log Out
                                </button>
                            </>
                            :
