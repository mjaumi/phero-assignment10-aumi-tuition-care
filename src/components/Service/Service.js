...
const Service = ({ service }) => {
    const { thumbnail, serviceName, studentClass, daysPerWeek, salary, negotiable } = service;
    return (
        <div className='border-2 border-tuition-care-base p-7 rounded-3xl'>
            ...
            <div className='mt-5'>
                <button className='bg-tuition-care-base-light border-2 border-tuition-care-base-light w-full py-3 rounded-xl text-white font-semibold text-lg hover:bg-transparent duration-300 hover:text-tuition-care-base-light'>Checkout</button>
            </div>
        </div>
    );
};
